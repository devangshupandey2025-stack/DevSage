...
```json
{
  ...
  "meta": { "limit": 20, "cursor": "seq:42", "has_more": true }
}
```
...
```ts
const cursor = c.req.query('cursor'); // e.g., "seq:42"
...
if (cursor) {
  const seq = parseInt(cursor.split(':')[1]);
  query += ` AND ae.sequence < ?`;
}

query += ` ORDER BY ae.sequence DESC LIMIT ?`;
```

Next cursor = `seq:{last_sequence}`. Client passes it as `?cursor=seq:42` to get the next page.
