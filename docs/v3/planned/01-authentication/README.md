...
- **SameSite**: `Lax` (default), `None` (for `.workers.dev` during dev), `Strict` (secure non-cross-site)
- **Secure**: `true` on HTTPS, `false` on HTTP (dev only)
- **Domain**: Per-subdomain (never `.devsage.org` — prevents cross-domain leakage)
