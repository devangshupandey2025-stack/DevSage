...
async function checkSubmissionDeadlines(db: DrizzleD1, env: Env) {
  ...
  for (const h of expired) {
    ...
    await stub.fetch(new Request('http://do/transition', {
      method: 'POST',
      body: JSON.stringify({ to: 'judging', triggered_by: 'cron' }),
    }));
    ...
    await insertAuditEvent(db, {
      hackathon_id: h.id,
      actor_type: 'cron',
      action: 'hackathon.auto_transition',
      details: { from: 'active', to: 'judging' },
    });
  }
}
```
...
- Cron has a 30-second CPU time limit on Workers (sufficient for these tasks)
