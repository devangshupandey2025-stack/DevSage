...
async acceptSubmission(body): Promise<{ accepted: boolean; reason?: string }> {
  ...
  // 5+6. Atomic lock + count update in DO SQLite transaction
  // DO SQLite supports BEGIN/COMMIT (unlike D1). We use EXCLUSIVE to prevent
  // concurrent reads during the lock.
  const now = new Date().toISOString();
  this.ctx.storage.sql.exec(`
    BEGIN EXCLUSIVE;
    INSERT INTO submission_locks (team_id, submission_key, locked_at)
    VALUES (?, ?, ?);
    INSERT OR REPLACE INTO team_submissions (team_id, count)
    VALUES (?, COALESCE((SELECT count FROM team_submissions WHERE team_id = ?), 0) + 1);
    COMMIT;
  `, body.team_id, body.submission_key, now, body.team_id, body.team_id);
...
> **Idempotency:** The `delivery_id` column in the `submissions` table MUST have a `UNIQUE` constraint. The INSERT should use `ON CONFLICT(delivery_id) DO NOTHING` to handle duplicate webhook deliveries.
