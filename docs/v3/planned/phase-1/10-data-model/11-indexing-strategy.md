...
| Index | Columns | Query Pattern |
|-------|---------|---------------|
| `idx_audit_hackathon_row` | `(hackathon_id, rowid)` | Audit feed: cursor-paginated chronological log for a hackathon. Primary query for the audit log page. Uses SQLite's implicit `rowid` for insertion order. |
...
- **Partial indexes supported**: D1 (SQLite 3.8.0+) supports partial indexes via `WHERE` clause. Example: `CREATE INDEX idx_audit_unhashed ON audit_events(hash) WHERE hash IS NULL` efficiently indexes only un-hashed events. Use partial indexes for filtering subsets of data.
...
- **Composite index ordering**: The leftmost column should be the most selective filter. For `(hackathon_id, rowid)`, hackathon_id narrows first, then rowid orders within.
