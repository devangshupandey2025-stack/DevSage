...
export default function ${componentName}({ hackathon }: { hackathon: any }) {
  const accent = hackathon?.primary_color ?? '#CCFF00';

  return (
    <div className="min-h-screen p-12">
      <div
        className="max-w-5xl mx-auto rounded-2xl border border-white/6 p-10"
        style={{
          background:
            'linear-gradient(135deg, rgba(255,255,255,0.02), rgba(255,255,255,0.01))',
        }}
      >
        <h1 className="text-5xl font-black" style={{ color: accent }}>
          {hackathon?.title}
        </h1>
        ...
        <div className="mt-8 text-sm text-white/60">
          Use this file to create a custom page for the hackathon.
        </div>
      </div>
    </div>
  );
