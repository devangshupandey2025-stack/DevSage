...
### Repository Addition/Removal (`installation_repositories` event)

When repos are added to or removed from an existing GitHub App installation:

- **`added`**: Same as new installation — upsert `team_repos` entries for matching teams
- **`removed`**: Deactivate `bot_active` for affected `team_repos` entries

```ts
case 'installation_repos_added':
  for (const repo of event.repositories) {
    const [owner, name] = repo.full_name.split('/');
    await db.run(sql`
      UPDATE team_repos SET bot_active = 1, github_installation_id = ${event.installation_id}
      WHERE github_owner = ${owner} AND github_repo = ${name}
    `);
  }
  break;
```

> **Note:** Subscribe to the `installation_repositories` event in your GitHub App webhook settings alongside `installation`.
