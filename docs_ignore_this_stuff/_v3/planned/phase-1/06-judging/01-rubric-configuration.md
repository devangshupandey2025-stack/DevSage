...
> **Data Protection:** The `scores` table references `rubric_criteria` with `ON DELETE RESTRICT` — a criterion that has scores cannot be deleted. The application must check for existing scores before allowing criterion deletion, or require archiving instead of deletion.
