...
The first platform admin must be seeded directly into D1 since the `addPlatformAdmin` endpoint requires existing admin auth. Use the Wrangler CLI:

```bash
# After first deploy, seed the initial admin (replace USER_ID with your user ID from the users table)
npx wrangler d1 execute devsage-db --command="INSERT INTO platform_admins (id, user_id, added_by, created_at) VALUES ('pa_bootstrap', 'YOUR_USER_ID', NULL, strftime('%Y-%m-%dT%H:%M:%fZ','now'));"
```

The `added_by` column is intentionally nullable for the bootstrapped admin.
