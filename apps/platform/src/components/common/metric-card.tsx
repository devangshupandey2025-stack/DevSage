...
export function MetricCard({ label, value, icon: Icon, change, changePositive, className }: MetricCardProps) {
  return (
    <motion.div
      ...
      className={cn(
        'group relative overflow-hidden rounded-2xl border border-white/ bg-white/2 p-5 transition-all duration-500 hover:border-white/12 hover:bg-white/4',
        className
      )}
    >
      {/* Subtle hover glow */}
      <div className="absolute -right-8 -top-8 h-24 w-24 rounded-full bg-[#CCFF00]/4 blur-2xl opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
