...
export async function consumeOAuthState(kv: KVNamespace, state: string): Promise<OAuthStateRecord | null> {
  ...
  try {
    ...
    if (!parsed.provider || !parsed.redirectUri || !parsed.createdAt) return null;
    return parsed;
  } catch {
    return null;
  }
...
export async function exchangeGoogleCodeForToken(params: {
  ...
  if (!token.access_token) throw new Error('Google token missing access token');
...
export async function fetchGoogleUserProfile(accessToken: string): Promise<GoogleOAuthProfile> {
  ...
  if (!response.ok) throw new Error('Google user info fetch failed');

  const user = (await response.json()) as GoogleUserResponse;
  if (!user.id || !user.email) throw new Error('Google user profile incomplete');
...
export async function exchangeGitHubCodeForToken(params: {
  ...
  if (!token.access_token) throw new Error('GitHub token missing access token');
  return token.access_token;
}
...
export async function fetchGitHubUserProfile(accessToken: string): Promise<GitHubOAuthProfile> {
  ...
  const userResponse = await fetchWithTimeout('https://api.github.com/user', { headers: baseHeaders });
  if (!userResponse.ok) throw new Error('GitHub user fetch failed');
  const user = (await userResponse.json()) as GitHubUserResponse;

  const emailResponse = await fetchWithTimeout('https://api.github.com/user/emails', { headers: baseHeaders });
  if (!emailResponse.ok) throw new Error('GitHub email fetch failed');
