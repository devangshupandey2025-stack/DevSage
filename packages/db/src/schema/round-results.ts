...
export const roundResults = sqliteTable('round_results', {
  ...
  uniqueRoundTeam: uniqueIndex('round_results_round_team_idx').on(table.round_id, table.team_id),
  idxHackathonStatus: index('round_results_hackathon_status_idx').on(table.hackathon_id, table.status),
  idxTeam: index('round_results_team_idx').on(table.team_id),
}));
