...
export const CreateHackathonRequestSchema = z.object({
  ...
  allowSolo: z.number().int().min(0).max(1).optional(),
  ...
  requireReadme: z.number().int().min(0).max(1).optional(),
  requireDemoUrl: z.number().int().min(0).max(1).optional(),
  allowRegistrationDuringActive: z.number().int().min(0).max(1).optional(),
  judgesPerSubmission: z.number().int().min(1).optional(),
  enableAiReviews: z.number().int().min(0).max(1).optional(),
  blindJudging: z.number().int().min(0).max(1).optional(),
  enableAudienceVoting: z.number().int().min(0).max(1).optional(),
  ...
  trackAssignmentMode: z.enum(['organizer_assigned', 'team_choice']).optional(),
  landingPagePublic: z.number().int().min(0).max(1).optional(),
